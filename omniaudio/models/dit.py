...
from .blocks import FourierFeatures, ChannelLastConv1d, ConvMLP, MLP
        ...
        if add_token_dim > 0:
            ...
            if use_conv:
                self.to_add_embed = nn.Sequential(
                    nn.SiLU(),
                    ConvMLP(add_token_dim, add_embed_dim, kernel_size=3, padding=1),
                )
            else:
                use_mlp = True
                self.to_add_embed = nn.Sequential(
                    nn.SiLU(),
                    MLP(add_token_dim, add_embed_dim),
                )
        else:
            self.add_embed_dim = 0
