...
class PadCrop_Video_Hiera_Normalized_T(nn.Module):
    ...
    def __call__(self, audio: torch.Tensor, video: torch.Tensor) -> Tuple[torch.Tensor, float, float, int, int]:
        ...
        #     video_chunk[0],
        #     size=(224, 224, 3), 
        #     scale_factor=(target_frames / video_tensor.shape[1], 1, 1),
        #     mode='trilinear', 
        #     align_corners=False
        # )
...
class FOA(nn.Module):
  def __call__(self, signal):
    ...
    if len(signal_shape) == 1:  # s -> (4, s)
        foa = torch.zeros(4, signal_shape[0], device=signal.device)  
        foa[0, :] = signal 
        foa[1, :] = 0  
        foa[2, :] = 0  
        foa[3, :] = 0  
    elif len(signal_shape) == 2:
        foa = torch.zeros(4, signal_shape[1], device=signal.device)  
        if signal_shape[0] == 1:  # (1, s) -> (4, s)
            ...
            foa[0, :] = (left + right) / np.sqrt(2)  
            foa[1,  :] = (left - right) / np.sqrt(2)  
            foa[2, :] = 0 
            foa[3, :] = 0 
        else:
            foa = signal
